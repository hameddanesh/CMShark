...
class DatasetIntegrator:
    ...
    def IntegrateDatasets(self, flowDirection, shuffleIt=True):
        ...
        myCursor.execute(
            'SELECT dataset_name,dataset_is_cm FROM datasets WHERE dataset_use=1 AND dataset_use_vpn=0 AND dataset_direction=' + str(flowDirection))
        ...
        # <write output>
        path = "flow_dg/outputs/netflow-dataset-" + \
            self._flowDirection[flowDirection]+".csv"
        ...
        outputHandler.close()
        print("\ndataset generated! flow direction: " +
              self._flowDirection[flowDirection]+"\n\n")
        # </write output>

    def FullIntegrate(self, shuffleIt=True):
        self.IntegrateDatasets(0)
        self.IntegrateDatasets(1)


if __name__ == "__main__":
    datasetIntegrator = DatasetIntegrator()
    datasetIntegrator.FullIntegrate()
